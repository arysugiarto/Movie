...
import com.arysugiarto.movies.data.repo.remote.ApiResponse
...
class Repo private constructor(
    ...
    companion object {
        ...
        fun getInstance(
            ...
            instance ?: synchronized(this) {
                instance ?: Repo(remoteData, localRepo, appExecutors)
            }
    }
    ...
    override fun getDetailMovie(movieId: String?): LiveData<Resource<MovieEntity>> {
        return object : NetworkBoundResource<MovieEntity, List<ModelMovie>>(appExecutors) {
            override fun loadFromDB(): LiveData<MovieEntity> = localRepo.getDetailMovie(movieId)
            override fun shouldFetch(data: MovieEntity?): Boolean = data == null
            override fun createCall(): LiveData<ApiResponse<List<ModelMovie>?>> = remoteDataSource.getDataMovie()
            override fun saveCallResult(data: List<ModelMovie>?) {
            }
    ...
    override fun getDetailTvShow(tvShowId: String?): LiveData<Resource<TvEntity>> {
        return object : NetworkBoundResource<TvEntity, List<ModelTvShow>>(appExecutors) {
            override fun loadFromDB(): LiveData<TvEntity> = localRepo.getDetailTv(tvShowId)
            override fun shouldFetch(data: TvEntity?): Boolean = data == null
            override fun createCall(): LiveData<ApiResponse<List<ModelTvShow>?>> = remoteDataSource.getDataTvShow()
            override fun saveCallResult(data: List<ModelTvShow>?) {
            }
